...
def train_net(net, cfg):
    ...
    # 学习率调整 milestones为一个数组 gamma是一个倍数
    scheduler = optim.lr_scheduler.MultiStepLR(optimizer,
                                               milestones=cfg.lr_decay_milestones,
                                               gamma = cfg.lr_decay_gamma)
    # lovasz-softmax loss是对语义分割指标miou直接进行优化的一个loss
    # lovasz-softmax loss是cvpr2018的一种基于iou的loss，效果优于cross_entropy，在分割任务中使用
    if cfg.n_classes > 1:
    ...
    for epoch in range(cfg.epochs):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{cfg.epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                global_step += 1
                # 满足条件调用验证集函数 并将验证的结果放入tensorboardx的summarywriter中
...
if __name__ == '__main__':
    ...
    if cfg.load:
        net.load_state_dict(
            torch.load("./data/checkpoints_v1/epoch_300.pth", map_location=device)
        )
